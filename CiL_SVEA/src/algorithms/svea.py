...
class SVEA(SAC):
	def __init__(self, obs_shape, action_shape, args):
		...
		head = m.HeadCNN(shared.out_shape, args.num_head_layers, args.num_filters).to(device)# .cuda()
		...
		self.actor = m.Actor(actor_encoder, action_shape, args.hidden_dim, args.actor_log_std_min, args.actor_log_std_max).to(device)# .cuda()
		self.critic = m.Critic(critic_encoder, action_shape, args.hidden_dim).to(device)# .cuda()
		self.critic_target = deepcopy(self.critic)

		self.log_alpha = torch.tensor(np.log(args.init_temperature)).to(device)# .cuda()
