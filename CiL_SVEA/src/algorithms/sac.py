...
class SAC(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		self.shared_cnn = m.SharedCNN(obs_shape, args.num_shared_layers, args.num_filters).to(self.device )
		rl_cnn = m.HeadCNN(self.shared_cnn.out_shape, args.num_head_layers, args.num_filters).to(self.device )
		...
		print('Shared:', utils.count_parameters(self.shared_cnn))
		print('Head:', utils.count_parameters(rl_cnn))
	...
	def update_critic(self, obs, action, reward, next_obs, Vh, Vh_next, not_done, L=None, step=None):
		with torch.no_grad():
			_, policy_action, log_pi, _ = self.actor([next_obs,Vh_next])
			target_Q1, target_Q2 = self.critic_target([next_obs,Vh_next], policy_action)
			target_V = torch.min(target_Q1,
								 target_Q2) - self.alpha.detach() * log_pi
			target_Q = reward + (not_done * self.discount * target_V)

		current_Q1, current_Q2 = self.critic([obs,Vh], action)
	...
	def update_actor_and_alpha(self, obs,Vh,  L=None, step=None, update_alpha=True):
		_, pi, log_pi, log_std = self.actor([obs,Vh], detach=True)
		actor_Q1, actor_Q2 = self.critic([obs,Vh], pi, detach=True)
	...
	def update(self, replay_buffer, L, step):
		obs, action, reward, next_obs, Vh,Vh_next, not_done = replay_buffer.sample()

		self.update_critic(obs, action, reward, next_obs, Vh, Vh_next, not_done, L, step)

		if step % self.actor_update_freq == 0:
			self.update_actor_and_alpha(obs,Vh, L, step)
